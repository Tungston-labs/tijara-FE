...
const fetchBuyers = async () => {
  try {
    console.log("Sending request to fetch buyers...");
    const response = await axios.get("http://localhost:5000/admin/auth/unapproved-users?role=buyer", {
      headers: {
        Authorization: `Bearer ${token}`,  // Ensure token is correctly passed
      },
    });
    console.log("Buyers data fetched successfully:", response.data);
    setBuyers(response.data);
  } catch (error) {
    console.error("Error fetching buyers data:", error.response ? error.response.data : error.message);
  ...
  const handleApprove = (userId) => {
    axios
      .post("http://localhost:5000/admin/auth/verify-user", {
        userId: buyer._id,
        role: "buyer",
        status: "approved",
      })
      .then(() => {
        setBuyers((prev) => prev.filter((user) => user._id !== buyer._id));
      })
    ...
    <div className="p-6 min-h-screen bg-[#E9E9E9] relative">
      ...
      <div className="max-w-6xl mx-auto mt-6 flex items-center justify-between text-sm">
        ...
        <div className="flex items-center gap-2">
          ...
          <button className="w-6 h-6 rounded-full bg-[#B3DB48] text-white">
            1
          </button>
          <button className="w-6 h-6 rounded-full bg-white">2</button>
          <button className="w-6 h-6 rounded-full bg-white">3</button>
