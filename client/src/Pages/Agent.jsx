...
export default function AgentTable() {
  ...
  // State to store editable values
  const [editName, setEditName] = useState('');
  const [editEmail, setEditEmail] = useState('');
  const [editPhone, setEditPhone] = useState('');
  const [editAddress, setEditAddress] = useState('');

  // Open the Edit modal and set initial values
  ...
  const handleOk = () => {
    console.log("Saved data:", { name: editName, email: editEmail, phone: editPhone, address: editAddress });
    setShowEditPopup(false); // Close modal after save
  };

  const handleCancel = () => {
    setShowEditPopup(false); // Close modal on cancel
  };
  ...
  return (
    <div className="min-h-screen bg-[#E9E9E9] p-6 relative overflow-hidden">
      ...
      {/* Edit Agent Modal using Ant Design */}
      <Modal
        title=""
        open={showEditPopup}
        onOk={handleOk}
